...
d3.csv("../exodus/csv/viz1-scatter-death.csv",rowConverter, function(data){
    ...
    var xScale = d3.scaleTime().domain([new Date("1961"),new Date("2016")]).range([0,w-padding*1.5]);
...
//                .attr("class","nodatatext");
    ...
    var line = d3.line().curve(d3.curveNatural).x(function(d){return xScale(d.date);}).y(function(d){ return yScale(d.refugees);});
    ...
    function showTooltip (d, i) {
        ...
        if(d.dead == 0){
            deadText = "Unknown deaths";
        }
        else{
            deadText = d.dead.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")+" civilian deaths";
        };
        ...
        wrapper
            ...
            .text((d.refugees).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")+" refugees & "+deadText)
            .transition().duration(200)
            .style("opacity", 0.5)
            .style("font-family","Inconsolata,monospace")
            .style("font-size","12px");	
    ...
    if (!mobileScreen) {
        ...
        var	legendMargin = {left: 5, top: 10, right: 5, bottom: 10},
            //legendWidth = 145,
            //legendWidth = 800,
            legendWidth = $("#wars-insights").width();
            legendHeight = 60;//270;
        ...
        var legend = legendWrapper.selectAll('.legendSquare')  	
                  ...
                  .attr("transform", function(d,i) { 
                      if(i<6){
                          return "translate("+(i * colHeight)+",0)";
                      }
                      else{
                          return "translate("+((i-6) * colHeight)+",25)";
                      }
                  })
